...
class Report:
    ...
    def _generate_csv(self) -> str:
        ...
        with open(csv_filename, "w", newline="") as csvfile:
            ...
            sorted_ips = sorted(
                self.capture.ip_packet_counter.items(),
                key=lambda x: x[1],
                reverse=True,
            )
    ...
    # Génère le rapport PDF complet : métadonnées, graphique, tableaux, analyse par protocole
    def _generate_pdf(self, filename: str) -> None:
        ...
        # Métadonnées : interface et total paquets
        ...
        # Graphique bar chart généré avec Pillow (PNG embarqué dans le PDF)
        pdf.set_font("Helvetica", "B", 12)
        pdf.cell(0, 8, "Graphique du trafic reseau",
                 new_x=XPos.LMARGIN, new_y=YPos.NEXT)
        ...
        try:
            ...
            # Largeur dynamique selon le nombre de protocoles (80px min par barre)
            bar_min_slot = 80
            W = max(750, margin_left + 20 + n * bar_min_slot)
            H = 350
        ...
        # Tableau des protocoles triés par nombre de paquets décroissant
        if self.capture.protocol_counter:
            pdf.set_font("Helvetica", "B", 12)
            pdf.cell(0, 8, "Tableau des protocoles",
                     new_x=XPos.LMARGIN, new_y=YPos.NEXT)
            pdf.set_font("Helvetica", "B", 10)
            pdf.cell(80, 7, "Protocole", border=1)
            pdf.cell(80, 7, "Nombre de paquets", border=1,
                     new_x=XPos.LMARGIN, new_y=YPos.NEXT)
            pdf.set_font("Helvetica", size=10)
            for proto, count in sorted(self.capture.protocol_counter.items(), key=lambda x: x[1], reverse=True):
                ...
                pdf.cell(80, 7, str(count), border=1,
                         new_x=XPos.LMARGIN, new_y=YPos.NEXT)
            pdf.ln(5)

        # Tableau des IPs avec détail des protocoles utilisés par chaque adresse
        if self.capture.ip_packet_counter:
            pdf.set_font("Helvetica", "B", 12)
            pdf.cell(0, 8, "Paquets par adresse IP",
                     new_x=XPos.LMARGIN, new_y=YPos.NEXT)
            pdf.set_font("Helvetica", "B", 10)
            pdf.cell(60, 7, "Adresse IP", border=1)
            pdf.cell(30, 7, "Total", border=1)
            pdf.cell(90, 7, "Detail par protocole", border=1,
                     new_x=XPos.LMARGIN, new_y=YPos.NEXT)
            pdf.set_font("Helvetica", size=10)
            ...
            for ip, count in sorted_ips:
                ...
                pdf.cell(90, 7, proto_details, border=1,
                         new_x=XPos.LMARGIN, new_y=YPos.NEXT)
            pdf.ln(5)

        # Analyse par protocole : statut légitime (vert) ou suspect (rouge) avec détail des alertes
        pdf.set_font("Helvetica", "B", 12)
        pdf.cell(0, 8, "Analyse par protocole",
                 new_x=XPos.LMARGIN, new_y=YPos.NEXT)

        proto_analysis = self.capture.get_proto_analysis()
        for proto, info in sorted(proto_analysis.items()):
            pdf.set_font("Helvetica", "B", 11)
            status_color = (200, 0, 0) if info["status"] == "SUSPICIOUS" else (0, 150, 0)
            pdf.set_text_color(*status_color)
            status_label = "SUSPECT" if info["status"] == "SUSPICIOUS" else "LEGITIME"
            ...
            if info["alerts"]:
                for alert in info["alerts"]:
                    ...
                    pdf.cell(0, 6, f"  {alert}",
                             new_x=XPos.LMARGIN, new_y=YPos.NEXT)
                pdf.set_text_color(0, 0, 0)
            else:
                pdf.set_text_color(0, 150, 0)
                pdf.cell(0, 6, "  Aucune activite suspecte detectee.",
                         new_x=XPos.LMARGIN, new_y=YPos.NEXT)
                pdf.set_text_color(0, 0, 0)
            pdf.ln(2)
        ...
        if not proto_analysis:
            pdf.set_font("Helvetica", size=10)
            pdf.set_text_color(0, 150, 0)
            pdf.cell(0, 7, "Tout le trafic est legitime.",
                     new_x=XPos.LMARGIN, new_y=YPos.NEXT)
            pdf.set_text_color(0, 0, 0)
